...
class BriotRuffiniDinamico(VoiceoverScene):
    def construct(self):
        self.set_speech_service(GTTSService(lang="pt", tld="com.br"), create_subcaption=False)
        ...
        # ------------------------------------------------
        # Offsets de layout: tabela à esquerda, gráfico à direita
        # ------------------------------------------------
        TAB_OFFSET = LEFT * 3.2
        ...
        with self.voiceover("Vamos aprender o Dispositivo de Briot-Ruffini para divisão de polinômios.", subcaption=False):
        ...
        with self.voiceover("Todo polinômio P de x pode ser escrito como o produto de x menos a pelo quociente Q de x, mais o resto R.", subcaption=False):
        ...
        with self.voiceover(f"O polinômio p de x de grau {grau} será avaliado no dispositivo com a igual a {fmt(a)}.", subcaption=False):
            self.play(Write(polinomio_label), run_time=1)
            self.play(Write(raiz_label), run_time=0.8)

        # ------------------------------------------------
        # Gráfico do polinômio (lado direito)
        # ------------------------------------------------
        ...
        with self.voiceover("À direita, vemos o gráfico do polinômio.", subcaption=False):
        ...
        with self.voiceover(f"O ponto destacado em amarelo indica o valor a igual a {fmt(a)} sobre o eixo x.", subcaption=False):
        ...
        # ------------------------------------------------
        # Tabela de Briot-Ruffini (lado esquerdo)
        # ------------------------------------------------
        CELL_W = 0.85
        ROW_H  = 0.65
        table_y = 0.3
        ...
        with self.voiceover("Construímos a tabela do dispositivo. À esquerda colocamos o valor de a, e na primeira linha os coeficientes do polinômio.", subcaption=False):
        ...
        with self.voiceover("O primeiro coeficiente desce diretamente para a linha do resultado.", subcaption=False):
        ...
        with self.voiceover("Agora, para cada coluna seguinte, multiplicamos o resultado anterior pelo valor de a e somamos com o coeficiente acima.", subcaption=False):
            for i in range(1, n):
        ...
        with self.voiceover("O último valor obtido é o resto da divisão, destacado em vermelho.", subcaption=False):
            self.play(Create(resto_rect), run_time=0.6)

        # Resultado final (abaixo da tabela)
        ...
        for j, c in enumerate(quociente_vals):
            ...
            if exp == 0:
                ...
                part_spk = (fmt(abs(c)) + " x") if abs(c) != 1 else "x"
            else:
                part_tex = (fmt(abs(c)) + f"x^{{{exp}}}") if abs(c) != 1 else f"x^{{{exp}}}"
                part_spk = (fmt(abs(c)) + f" x elevado a {exp}") if abs(c) != 1 else f"x elevado a {exp}"
            if j == 0:
                q_str  += ("-" if c < 0 else "") + part_tex
                q_spoken += ("menos " if c < 0 else "") + part_spk
            else:
                q_str  += (" - " if c < 0 else " + ") + part_tex
                q_spoken += (" menos " if c < 0 else " mais ") + part_spk
        ...
        resto_str = fmt(resto_val)
        if resto_val == 0:
            voiceover_final = f"O quociente Q de x é {q_spoken} e o resto é zero, confirmando que {fmt(a)} é raiz do polinômio."
        else:
            voiceover_final = f"O quociente Q de x é {q_spoken} e o resto R é {resto_str}."

        with self.voiceover(voiceover_final, subcaption=False):
